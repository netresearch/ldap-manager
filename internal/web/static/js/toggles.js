...
function updateThemeButtonState(button, mode) {
...
export function initDensityToggle() {
    ...
    if (!button)
        return;
    // Set initial state from storage
    const currentMode = localStorage.getItem(DENSITY_STORAGE_KEY) || "auto";
    updateDensityButtonState(button, currentMode);
    button.addEventListener("click", () => {
        const current = localStorage.getItem(DENSITY_STORAGE_KEY) || "auto";
...
function getNextDensityMode(current) {
    switch (current) {
        case "auto":
            return "comfortable";
        case "comfortable":
...
function updateDensityButtonState(button, mode) {
...
function determineActualDensity(mode) {
    if (mode === "comfortable")
        return "comfortable";
    if (mode === "compact")
        return "compact";
