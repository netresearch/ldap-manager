...
try {
  ...
  await import(ASSETS + (kind === 'wizard' ? '/wizard-login.js' : kind === 'keyholder' ? '/keyholder-login.js' : '/scormiq-avatar.js'));
  form.addEventListener('submit', event => {
    if (!event.defaultPrevented) { submissions++; event.preventDefault(); }
  });
  await wait(250);
  ...
  if (kind === 'keyholder') {
    ...
    const keyCenter = new THREE.Box3().setFromObject(avatar._rig.keys).getCenter(new THREE.Vector3()).project(avatar._camera);
    const rect = canvas.getBoundingClientRect();
    const click = () => canvas.dispatchEvent(new MouseEvent('click', {bubbles:true, detail:1, clientX:rect.left+(keyCenter.x+1)/2*rect.width, clientY:rect.top+(1-keyCenter.y)/2*rect.height}));
    click();
